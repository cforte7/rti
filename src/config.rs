pub mod config {
    ...
    pub fn set_tz_config(tz_input: Option<String>) -> OkOrStringError {
        ...
        let timezone: Tz = match tz_input {
            Some(val) => match val.parse() {
                Ok(v) => v,
                Err(e) => return Err("Invalid timezone provided.")
            },
            None => return Err("Must provide timezone argument.")
        };
        ...
        match confy::store("rti", new_config) {
            Ok(_) => Ok(Some(format!("Timezone updated to {}", timezone).as_str())),
            Err(e) => {
                Err(format!("Error storing timezone: {}", e.to_string()).as_str())
            }
        }
    }
    ...
    pub fn add_custom_token(new_token: Option<String>) -> OkOrStringError {
        ...
        match confy::store("rti", new_config) {
            Ok(_) => Ok(Some("Custom Token successfully added.")),
            Err(e) => {
                Err(format!("Error storing custom token: {}", e.to_string()).as_str())
            }
        }
    ...
    pub fn remove_custom_token(to_remove: Option<String>) -> OkOrStringError {
        let token: String = match to_remove {
            Some(val) => val,
            None => return Err("Must provide token to remove.")
        };
        let existing_tokens: Vec<String> = match load_config().custom_parsing_tokens {
            Some(tokens) => tokens,
            None=>{
                return Ok(Some("No tokens to remove."));
            }
        };
        let filtered_tokens: Vec<String> = existing_tokens
            ...
            .collect();
        if let False = filtered_tokens.len() > 0 {
            return Ok(Some("No matching token found."));
        }
        ...
        match confy::store("rti", new_config) {
            Ok(_) => Ok(Some("Custom token removed.")),
            Err(e) => {
                Err(format!("Error removing custom token: {}", e.to_string()).as_str())
            }
        }
    }
    ...
    pub fn view_tokens() -> OkOrStringError {
        ...
        match load_config().custom_parsing_tokens {
            Some(tokens) => existing_tokens.append(&mut tokens),
            None => return Err("No custom tokens exist!")
        };
